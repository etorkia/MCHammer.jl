...
function learn_rate(::CrawfordMethod,LotUnitsA, WorkUnitsA, LotUnitsB, WorkUnitsB)
    Mid_Lot_A =  ((LotUnitsA+1)/3)+0.5
    Mid_Lot_B = LotUnitsB/2+LotUnitsA

    Mid_Work_A = WorkUnitsA/LotUnitsA
    Mid_Work_B = ((WorkUnitsB + WorkUnitsA)-WorkUnitsA)/LotUnitsB

    SolveA = round((log(Mid_Work_A)-log(Mid_Work_B))/(log(Mid_Lot_B)-log(Mid_Lot_A)), digits=4)
    SolveB = round(Mid_Work_A*(1.5^SolveA), digits=4)

    println("Incremental time function = Y=$SolveB", "X^","$SolveA")

    LogLearnRate = (log(2) / log(10)) * -SolveA
   
    LearningRate = exp(LogLearnRate)
